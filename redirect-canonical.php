...
add_action( 'init', function() {
	...
	add_action( 'template_redirect', 'swpd_redirect_canonical' );
}, 10, 0 );

function swpd_redirect_canonical_log( $location, $redirect_url ) {
	$message = '$redirect_url is being set via ' . $location;
	swpd_log( 'redirect_canonical', $message, array( 'redirect_url' => $redirect_url ), wp_debug_backtrace_summary() );
}
 ...
 */
function swpd_redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	if ( is_feed() && ( $id = get_query_var( 'p' ) ) ) {
		if ( $redirect_url = get_post_comments_feed_link( $id, get_query_var( 'feed' ) ) ) {
			swpd_redirect_canonical_log( '#1', $redirect_url );
	...
	if ( is_singular() && 1 > $wp_query->post_count && ($id = get_query_var('p')) ) {
		...
		if ( isset($vars[0]) && $vars = $vars[0] ) {
			...
			if ( $redirect_url = get_permalink($id) ) {
				...
				swpd_redirect_canonical_log( '#2', $redirect_url );
			}
		}
	}
	...
	if ( is_404() ) {
		...
		if ( $id && $redirect_post = get_post($id) ) {
			...
			if ( $post_type_obj->public && 'auto-draft' != $redirect_post->post_status ) {
				...
				swpd_redirect_canonical_log( '#3', $redirect_url );
				$redirect['query'] = _remove_qs_args_if_not_in_url( $redirect['query'], array( 'p', 'page_id', 'attachment_id', 'pagename', 'name', 'post_type' ), $redirect_url );
			}
		}
		...
		if ( get_query_var( 'day' ) && get_query_var( 'monthnum' ) && get_query_var( 'year' ) ) {
			...
			if ( ! wp_checkdate( $month, $day, $year, $date ) ) {
				...
				swpd_redirect_canonical_log( '#4', $redirect_url );
			...
			swpd_redirect_canonical_log( '#5', $redirect_url );
		...
		if ( ! $redirect_url ) {
			if ( $redirect_url = redirect_guess_404_permalink() ) {
				swpd_redirect_canonical_log( '#6', $redirect_url );
		...
		if ( get_query_var( 'page' ) && $wp_query->post &&
			...
			swpd_redirect_canonical_log( '#7', $redirect_url );
		}
		...
		if ( is_attachment() &&
			...
			if ( ! empty( $_GET['attachment_id'] ) ) {
				...
				swpd_redirect_canonical_log( '#8', $redirect_url );
				if ( $redirect_url ) {
					$redirect['query'] = remove_query_arg( 'attachment_id', $redirect['query'] );
				}
			} else {
				$redirect_url = get_attachment_link();
				swpd_redirect_canonical_log( '#9', $redirect_url );
			}
		} elseif ( is_single() && !empty($_GET['p']) && ! $redirect_url ) {
			if ( $redirect_url = get_permalink(get_query_var('p')) ) {
				swpd_redirect_canonical_log( '#10', $redirect_url );
				$redirect['query'] = remove_query_arg( array( 'p', 'post_type' ), $redirect['query'] );
			}
		} elseif ( is_single() && !empty($_GET['name'])  && ! $redirect_url ) {
			if ( $redirect_url = get_permalink( $wp_query->get_queried_object_id() ) ) {
				swpd_redirect_canonical_log( '#11', $redirect_url );
				$redirect['query'] = remove_query_arg( 'name', $redirect['query'] );
			}
		} elseif ( is_page() && !empty($_GET['page_id']) && ! $redirect_url ) {
			if ( $redirect_url = get_permalink(get_query_var('page_id')) ) {
				swpd_redirect_canonical_log( '#12', $redirect_url );
			...
			swpd_redirect_canonical_log( '#13', $redirect_url );
			...
			if ( $redirect_url = get_permalink(get_option('page_for_posts')) ) {
				swpd_redirect_canonical_log( '#14', $redirect_url );
				$redirect['query'] = remove_query_arg( 'page_id', $redirect['query'] );
			}
			...
			switch ( strlen($m) ) {
				case 4: // Yearly
					...
					swpd_redirect_canonical_log( '#15', $redirect_url );
					break;
				case 6: // Monthly
					$redirect_url = get_month_link( substr($m, 0, 4), substr($m, 4, 2) );
					swpd_redirect_canonical_log( '#16', $redirect_url );
					break;
				case 8: // Daily
					$redirect_url = get_day_link(substr($m, 0, 4), substr($m, 4, 2), substr($m, 6, 2));
					swpd_redirect_canonical_log( '#17', $redirect_url );
					break;
			}
			if ( $redirect_url )
			...
			if ( $redirect_url = get_day_link(get_query_var('year'), get_query_var('monthnum'), get_query_var('day')) ) {
				swpd_redirect_canonical_log( '#18', $redirect_url );
			...
			if ( $redirect_url = get_month_link(get_query_var('year'), get_query_var('monthnum')) ) {
				swpd_redirect_canonical_log( '#19', $redirect_url );
				$redirect['query'] = remove_query_arg( array( 'year', 'monthnum' ), $redirect['query'] );
			}
		} elseif ( is_year() && !empty($_GET['year']) ) {
			if ( $redirect_url = get_year_link(get_query_var('year')) ) {
				swpd_redirect_canonical_log( '#20', $redirect_url );
			...
			if ( ( false !== $author ) && $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE $wpdb->posts.post_author = %d AND $wpdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) ) ) {
				if ( $redirect_url = get_author_posts_url($author->ID, $author->user_nicename) ) {
					swpd_redirect_canonical_log( '#21', $redirect_url );
			...
			if ( ( ! $category || is_wp_error( $category ) ) || ! has_term( $category->term_id, 'category', $wp_query->get_queried_object_id() ) ) {
				...
				swpd_redirect_canonical_log( '#22', $redirect_url );
			}
		}
		...
		if ( is_singular() && get_query_var('page') ) {
			if ( !$redirect_url ) {
				...
				swpd_redirect_canonical_log( '#23', $redirect_url );
			}
			...
			if ( $page > 1 ) {
				if ( is_front_page() ) {
					...
					swpd_redirect_canonical_log( '#24', $redirect_url );
				} else {
					$redirect_url = trailingslashit( $redirect_url ) . user_trailingslashit( $page, 'single_paged' );
					swpd_redirect_canonical_log( '#25', $redirect_url );
				}
			}
			$redirect['query'] = remove_query_arg( 'page', $redirect['query'] );
		}
		...
		if ( get_query_var('paged') || is_feed() || get_query_var('cpage') ) {
			...
			if ( is_feed() && in_array( get_query_var('feed'), $wp_rewrite->feeds ) ) {
				...
				if ( isset( $old_feed_files[ basename( $redirect['path'] ) ] ) ) {
					...
					swpd_redirect_canonical_log( '#26', $redirect_url );
			...
			swpd_redirect_canonical_log( '#27', $redirect_url );
		}
		...
		if ( 'wp-register.php' == basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				...
				swpd_redirect_canonical_log( '#28', $redirect_url );
			} else {
				$redirect_url = wp_registration_url();
				swpd_redirect_canonical_log( '#29', $redirect_url );
			}
	...
	if ( $redirect_url && !empty($redirect['query']) ) {
		...
		swpd_redirect_canonical_log( '#30', $redirect_url );
	}
	...
	if ( $compare_original !== $compare_redirect ) {
		...
		swpd_redirect_canonical_log( '#31', $redirect_url );
	}
	...
	swpd_redirect_canonical_log( '#32', $redirect_url );
